...
function calculateFormatting(text: string): number {
  let score = 10;
...
function generateQuickStrengths(resume: any): string[] {
  const strengths: string[] = [];
  if (resume.skills.length > 0) strengths.push('Skills section is present');
  if (resume.experience.length > 0) strengths.push('Experience details are included');
  if (resume.education.length > 0) strengths.push('Education background is listed');
  return strengths;
}

function generateQuickWeaknesses(resume: any): string[] {
  const weaknesses: string[] = [];
  if (resume.skills.length === 0) weaknesses.push('No clear skills section found');
  if (resume.experience.length === 0) weaknesses.push('Experience section needs expansion');
  if (resume.education.length === 0) weaknesses.push('Education section is missing');
  return weaknesses;
}
