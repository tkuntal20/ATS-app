export function calculateATSScore(resume, jd) {
    const breakdown = {
...
function calculateSkillAlignment(resume, jd) {
...
function calculateExperienceMatch(resume, jd) {
...
function calculateReadability(resume) {
    ...
    const hasActionVerbs = /^(achieved|created|developed|designed|implemented|led|managed|organized|reduced)/m.test(resume.rawText);
    if (!hasActionVerbs) {
        score -= 3;
    }
    return Math.max(5, score);
}
function findSkillMatches(resume, jd) {
    const resumeSkills = resume.skills.map((s) => s.toLowerCase());
    const requiredSkills = jd.requiredSkills.map((s) => s.toLowerCase());
    const matched = requiredSkills.filter((skill) => resumeSkills.some((rs) => rs.includes(skill) || skill.includes(rs)));
    const missing = requiredSkills.filter((skill) => !resumeSkills.some((rs) => rs.includes(skill) || skill.includes(rs)));
    return { matchedSkills: matched, missingSkills: missing };
}
function generateInsights(resume, jd, breakdown) {
    const strengths = [];
    const weaknesses = [];
